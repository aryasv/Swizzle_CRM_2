...
class WebFunctions {
  ...
  Future<ApiResponse> getCompanyDeals({
    required BuildContext context,
    required String companyUuid,
    required int companyId,
  ...
  Future<ApiResponse> addCompanyNote({
    ...
    print("Add Note API - Adding note for company: $companyUuid with ${attachments?.length ?? 0} attachments");

    if (attachments != null && attachments.isNotEmpty) {
      // Use multipart request for attachments
       if (await checkConnection()) {
        try {
          final url = Uri.parse("${baseURL}company/$companyUuid/notes/store");
          final signUrl = "${baseURL}company/$companyUuid/notes/store";
      ...
      // Use standard post request if no attachments
      return await callApiFunction(
        context,
